    ...
    for (j = 0; j < 4; j++) {
		...
		for(i=0;i<2;i++){
			...
			if (strlen(buffer) < (sizeof(buffer)-1)) {
				strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));	//Fix for 108070
			}
    	}
		//strcat(buffer,"|====================|");
		if (strlen(buffer) < (sizeof(buffer)-1))
			strncat(buffer,"|====================|",(sizeof(buffer)-1-strlen(buffer)));	//Fix for 108071
		ErrorPrint(NartOther, buffer);
    }
	SformatOutput(buffer, MBUFFER-1," |=======================================================================|");
	ErrorPrint(NartOther, buffer);
...
void print9287_2GLegacyTargetPower(int client, const CAL_TARGET_POWER_LEG *pVals)
{
    ...
    for (j = 0; j < 4; j++) {
		...
		for(i=0;i<3;i++){
			...
			//strcat(buffer,buffer2);
			if (strlen(buffer) < (sizeof(buffer)-1))
				strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));	//Fix for 108072
    	}

		//strcat(buffer,"|");
		if (strlen(buffer) < (sizeof(buffer)-1))
			strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));		//Fix for 108073
		ErrorPrint(NartOther, buffer);
    }
	SformatOutput(buffer, MBUFFER-1," |=======================================================================|");
	ErrorPrint(NartOther, buffer);
...
void print9287_2GHT20TargetPower(int client, const CAL_TARGET_POWER_HT *pVals) {
    ...
    //for (j = 0; j < 8; j++) {
		...
		for(i=0;i<3;i++){
			...
			//strcat(buffer,buffer2);
			if (strlen(buffer) < (sizeof(buffer)-1))
				strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//Fix for 108074
        }

		if (strlen(buffer) < (sizeof(buffer)-1))
			strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));	//Fix for 108075
		ErrorPrint(NartOther, buffer);
    }
	SformatOutput(buffer, MBUFFER-1," |=======================================================================|");
	ErrorPrint(NartOther, buffer);
...
void print9287_2GHT40TargetPower(int client, const CAL_TARGET_POWER_HT *pVals) {
    ...
    for (j = 0; j < 16; j++) {
		...
		for(i=0;i<3;i++){
			...
			//SformatOutput(buffer2, MBUFFER-1,"|        %2d.%d        ", (pVals+i)->tPow2x[j]/2, ((pVals+i)->tPow2x[j] % 2) * 5);
			if (strlen(buffer) < (sizeof(buffer)-1))
				strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//Fix for 108076
        }

		if (strlen(buffer) < (sizeof(buffer)-1))
			strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));		//Fix for 108077
		ErrorPrint(NartOther, buffer);
    }
	SformatOutput(buffer, MBUFFER-1," |=======================================================================|");
	ErrorPrint(NartOther, buffer);
