...
#undef NO_SPUR
    ...
    for( i = 0; i < 1; i++ ) 
    {
        for( c = 0; c < AR9287_MAX_CHAINS; c++ ) 
        {
            ...
            for( channelRowCnt = 0; channelRowCnt < numChannels; channelRowCnt += 5 ) 
            {
			    ...
			    //printf("\nFix for 108159 , 108160 report-2\n");
			    if(k<2){
    ...
    for( i = 0; i < 7; i++ ) 
    {
        ...
        for( j = 0; j < numPowers; j+=4 ) 
        {
			...
			strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));	//Fix for 108162
                }
                else 
                {
                    numPowers = k;
            ...
            SformatOutput(buffer2, MBUFFER-1,"|");
	    if (strlen(buffer) < (sizeof(buffer)-1))
		strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));	//Fix for 108163
            //SendIt(client,buffer);
			...
			if(r<4)
			{
				...
				if (strlen(buffer) < (sizeof(buffer)-1))
                    ...
                    else 
                    {
                        ...
                        //SendIt(client,buffer);
			if (strlen(buffer) < (sizeof(buffer)-1))
                		strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//fix for 108165
                    }
                }
                SformatOutput(buffer2, MBUFFER-1,"|");
		if (strlen(buffer) < (sizeof(buffer)-1))
	                strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//fix for 108166
                //SendIt(client,buffer);
            }
            SformatOutput(buffer, MBUFFER-1,"|==============|==============|==============|==============|==============|");
            //SendIt(client,buffer);
