...
struct _ParameterList Qc98xx_GetParameter[]=
{
    //
    ...
    QC98XX_SET_FEATURE_ENABLE_SMART_ANTENNA,
    ...
    QC98XX_SET_BOARD_FLAG1_TX_BURST,
...
//    QC98XX_SET_2GHZ_NOISE_FLOOR_THRESHOLD,
    ...
    QC98XX_SET_2GHZ_COARSE_INDEX,
    QC98XX_SET_2GHZ_FINE_INDEX,
    QC98XX_SET_2GHZ_ADC_DESIRED_SIZE,
    QC98XX_SET_2GHZ_QUICK_DROP,
    QC98XX_SET_2GHZ_MAX_RETRAIN_TIMES,
    QC98XX_SET_2GHZ_SETTLE_TIME,
...
//    QC98XX_SET_5GHZ_NOISE_FLOOR_THRESHOLD,
    ...
    QC98XX_SET_5GHZ_COARSE_INDEX,
    QC98XX_SET_5GHZ_FINE_INDEX,
    QC98XX_SET_5GHZ_ADC_DESIRED_SIZE,
    QC98XX_SET_5GHZ_QUICK_DROP,
    QC98XX_SET_5GHZ_MAX_RETRAIN_TIMES,
    QC98XX_SET_5GHZ_SETTLE_TIME,	
...
//	QC98XX_SET_5GHZ_NOISE_FLOOR_TEMPERATURE
    ...
    QC98XX_SET_2GHZ_OBDB_AUX1_AUX2,
    QC98XX_SET_5GHZ_OBDB_AUX1_AUX2_LOW,
    QC98XX_SET_5GHZ_OBDB_AUX1_AUX2_MID,
    QC98XX_SET_5GHZ_OBDB_AUX1_AUX2_HIGH,
...
{
    list->nspecial=sizeof(Qc98xx_GetParameter)/sizeof(struct _ParameterList);
    list->special=Qc98xx_GetParameter;
    return 0;
}

QC98XXDLLSPEC int Qc98xxSetParameterSplice(struct _ParameterList *list)
{
    list->nspecial=(sizeof(Qc98xx_GetParameter)/sizeof(struct _ParameterList))-1;
    list->special=&Qc98xx_GetParameter[1];
    return 0;
}
...
static void ReturnGet(int ix, int iy, int iz, int status, int *done, int *error, int ip, int index, int *value, int num)
{
    ...
    if(status==VALUE_OK)
    {
        if (Qc98xx_GetParameter[index].type=='u' || Qc98xx_GetParameter[index].type=='z') {
			ReturnUnsigned("get",Qc98xx_GetParameter[index].word[0],atext,value,num, Qc98xx_GetParameter[index].units);
        } else if (Qc98xx_GetParameter[index].type=='d') {
            ReturnSigned("get",Qc98xx_GetParameter[index].word[0],atext,value,num);
        } else if (Qc98xx_GetParameter[index].type=='x') {
            ReturnHex("get",Qc98xx_GetParameter[index].word[0],atext,value,num);
        }
        (*done)+=1;
    } else {	// ErrorPrint();
        (*error)+=1;
...
static void ReturnGetf(int ix, int iy, int iz, int status, int *done, int *error, int ip, int index, double *value, int num)
{
    ...
    if(status==VALUE_OK)
    {
            ReturnDouble("get",Qc98xx_GetParameter[index].word[0],atext,value,num);
        (*done)+=1;
    } else {	// ErrorPrint();
        (*error)+=1;
    }
                            ...
                            int (*_pSetsBand)(int *value, int ix, int iy, int iz, int num, int iBand))
{
    ...
    if (Qc98xx_GetParameter[index].type=='u' || Qc98xx_GetParameter[index].type=='z') {
        ngot=ParseIntegerList(ip,name,uvalue,&Qc98xx_GetParameter[index]);
        if(ngot>0) {
            status=_pSetsBand(uvalue, ix, iy, iz, ngot, iband);
            if(status==VALUE_OK)
                ReturnUnsigned("set",Qc98xx_GetParameter[index].word[0],atext,uvalue,ngot, Qc98xx_GetParameter[index].units);
        }
    } else if (Qc98xx_GetParameter[index].type=='d') {
        ngot=ParseIntegerList(ip,name,value,&Qc98xx_GetParameter[index]);
        if(ngot>0) {
            status=_pSetsBand(value, ix, iy, iz, ngot, iband);
            if(status==VALUE_OK)
                ReturnSigned("set",Qc98xx_GetParameter[index].word[0],atext,value,ngot);
        }
    } else if (Qc98xx_GetParameter[index].type=='x') {
        ngot=ParseHexList(ip,name,uvalue,&Qc98xx_GetParameter[index]);
        if(ngot>0) {
            status=_pSetsBand(uvalue, ix, iy, iz, ngot, iband);
            if(status==VALUE_OK)
                ReturnHex("set",Qc98xx_GetParameter[index].word[0],atext,uvalue,ngot);
        }
    }
    if (ngot<=0) {
        (*error)+=1;
                            ...
                            int (*_pfSetsBand)(double *value, int ix, int iy, int iz, int num, int iBand))
{
    ...
    name=CommandParameterName(ip);
    ngot=ParseDoubleList(ip,name,value,&Qc98xx_GetParameter[index]);
    if(ngot>0) {
        status=_pfSetsBand(value, ix, iy, iz, ngot, iband);
        if(status==VALUE_OK)
            ReturnDouble("set",Qc98xx_GetParameter[index].word[0],atext,value,ngot);
    }
	*error=status;
}
                            ...
                            int (*_pSet)(int value), int (*_pSetBand)(int value, int iBand))
{
    ...
    if (Qc98xx_GetParameter[index].type=='u' || Qc98xx_GetParameter[index].type=='z') {
        ...
        if(ngot>0) {
            ...
            if(status==VALUE_OK)
                ReturnUnsigned("set",Qc98xx_GetParameter[index].word[0],"",uvalue,ngot,Qc98xx_GetParameter[index].units);
        }
    } else if (Qc98xx_GetParameter[index].type=='d') {
        ngot=ParseIntegerList(ip,name,value,&Qc98xx_GetParameter[index]);
        if(ngot>0) {
            ...
            if(status==VALUE_OK)
                ReturnSigned("set",Qc98xx_GetParameter[index].word[0],"",value,ngot);
        }
    } else if (Qc98xx_GetParameter[index].type=='x') {
        ngot=ParseHexList(ip,name,uvalue,&Qc98xx_GetParameter[index]);
        if(ngot>0) {
            if (iband==-1)
                ...
                status=_pSetBand(uvalue[0], iband);
            if(status==VALUE_OK)
                ReturnHex("set",Qc98xx_GetParameter[index].word[0],"",uvalue,ngot);
        }
    }
    if (ngot<=0) {
        (*error)+=1;
...
QC98XXDLLSPEC int Qc98xxSetCommand(int client)
{
    ...
    {
        name=CommandParameterName(ip);
        index=ParameterSelectIndexArray(name,&Qc98xx_GetParameter[1],sizeof(Qc98xx_GetParameter)/sizeof(Qc98xx_GetParameter[1])-1, &ix, &iy, &iz);
        if(index>=0)
        {

            index++;
            code=Qc98xx_GetParameter[index].code;
            switch(code)
            {
					...
					if (parseStatus==1)
                    {
						...
						if (parseStatus==1)
                        {
                            ...
                            else
                            {
							    ...
							    ReturnText("set",Qc98xx_GetParameter[index].word[0],"",buffer);
                            }
						}
					}
					break;
                ...
                case Qc9KSetEepromCustomerData:
                    ...
                    if(status==VALUE_OK)
                    {
                        ReturnText("set",Qc98xx_GetParameter[index].word[0],"",text);
                        done++;
                    }
                    else	// ErrorPrint();
                        error++;
                ...
                case Qc9KSetEepromFeatureEnableTuningCaps:
                    ...
                    break;
                case Qc9KSetEepromFeatureEnableSmartAntenna:
                    ConfigSet(&done, &error, ip, index, -1, Qc98xxEnableSmartAntennaSet, 0);
                    break;
				...
				case Qc9KSetEepromFlag1NoiseFlrThr:
                    ...
                    break;
                case Qc9KSetEepromFlag1TxBurst:
                    ConfigSet(&done, &error, ip, index, -1, Qc98xxFlag1TxBurstSet, 0);
                    break;
				...
				case Qc9KSetEeprom2GHzMinCcaPwrChain:
					...
					break;
                case Qc9KSetEeprom2GHzCoarseIndex:
                    ConfigSet(&done, &error, ip, index, band_BG, 0, Qc98xxCoarseIndexSet);
                    break;
                case Qc9KSetEeprom2GHzFineIndex:
                    ConfigSet(&done, &error, ip, index, band_BG, 0, Qc98xxFineIndexSet);
                    break;
                case Qc9KSetEeprom2GHzAdcDesiredSize:
                ...
                case Qc9KSetEeprom2GHzQuickDrop:
                    ...
                    break;
                case Qc9KSetEeprom2GHzMaxRetrainTimes:
                    ConfigSet(&done, &error, ip, index, band_BG, 0, Qc98xxMaxRetrainTimesSet);
                    break;
                case Qc9KSetEeprom2GHzSettleTime:
                    ConfigSet(&done, &error, ip, index, band_BG, 0, Qc98xxSettleTimeSet);
                    break;
                ...
                case Qc9KSetEeprom2GHzXatten1Margin:
                    ...
                    break;
                case Qc9KSetEeprom2GHzObdbAux1Aux2:
                    ConfigSets(&done, &error, ip, index, ix, iy, iz, band_BG, Qc98xxObdbAux1Aux2Set);
                    break;                    
				...
				case Qc9KSetEeprom5GHzMinCcaPwrChain:
					...
					break;
                case Qc9KSetEeprom5GHzCoarseIndex:
                    ConfigSet(&done, &error, ip, index, band_A, 0, Qc98xxCoarseIndexSet);
                    break;
                case Qc9KSetEeprom5GHzFineIndex:
                    ConfigSet(&done, &error, ip, index, band_A, 0, Qc98xxFineIndexSet);
                    break;
                case Qc9KSetEeprom5GHzAdcDesiredSize:
                ...
                case Qc9KSetEeprom5GHzQuickDrop:
                    ...
                    break;
                case Qc9KSetEeprom5GHzMaxRetrainTimes:
                    ConfigSet(&done, &error, ip, index, band_A, 0, Qc98xxMaxRetrainTimesSet);
                    break;
                case Qc9KSetEeprom5GHzSettleTime:
                    ConfigSet(&done, &error, ip, index, band_A, 0, Qc98xxSettleTimeSet);
                    break;                    
                ...
                case Qc9KSetEeprom5GHzXatten1MarginMid:
                    ...
                    break;
                case Qc9KSetEeprom5GHzObdbAux1Aux2Mid:
                    ConfigSets(&done, &error, ip, index, ix, iy, iz, band_A, Qc98xxObdbAux1Aux2Set);
                    break;
                ...
                case Qc9KSetEeprom5GHzXatten1MarginHigh:
                    ...
                    break;
                case Qc9KSetEeprom5GHzObdbAux1Aux2Low:
                    ConfigSets(&done, &error, ip, index, ix, iy, iz, band_A, Qc98xxObdbAux1Aux2LowSet);
                    break;
                case Qc9KSetEeprom5GHzObdbAux1Aux2High:
                    ConfigSets(&done, &error, ip, index, ix, iy, iz, band_A, Qc98xxObdbAux1Aux2HighSet);
                    break;                    
...
QC98XXDLLSPEC int Qc98xxGetCommand(int client)
{
    ...
    {
        name=CommandParameterName(ip);
        index=ParameterSelectIndexArray(name,&Qc98xx_GetParameter[0],sizeof(Qc98xx_GetParameter)/sizeof(Qc98xx_GetParameter[0]), &ix, &iy, &iz);
        if(index>=0)
        {
            code=Qc98xx_GetParameter[index].code;
            buffer[0]=0;
            switch(code)
            {
				...
				case Qc9KSetEepromConfig:
					...
					if(ngot-1 > 0)
					{
						ReturnHex("get",Qc98xx_GetParameter[index].word[0],"",(int *)uvalue,(ngot-1)*2);
						done++;
					}
					else
					{
				...
				case Qc9KSetEepromConfigPCIe:
					...
					if(ngot-1 > 0) {
						ReturnHex("get",Qc98xx_GetParameter[index].word[0],"",uvalue,(ngot-1)*2);
						done++;
					} else
                ...
                case Qc9KSetEepromMacAddress:
                    ...
                    if(status==VALUE_OK)
                    {
                        ...
                        ReturnText("get",Qc98xx_GetParameter[index].word[0],"",buffer);
                        done++;
                    }
                    else
                    {	// ErrorPrint();
                ...
                case Qc9KSetEepromFeatureEnableTuningCaps:
                    ...
                    break;
                case Qc9KSetEepromFeatureEnableSmartAntenna:
                    value[0]=Qc98xxEnableSmartAntennaGet();
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEepromMiscellaneous:
                ...
                case Qc9KSetEepromFlag1NoiseFlrThr:
                    ...
                    break;
                case Qc9KSetEepromFlag1TxBurst:
                    status=Qc98xxFlag1TxBurstGet(value);
                    ReturnGet(ix,iy,iz, status, &done, &error, ip, index, value, 1);
                    break;
				case Qc9KSetEepromWlanLedGpio:
				...
				case Qc9KSetEeprom2GHzMinCcaPwrChain:
					...
					break;
                case Qc9KSetEeprom2GHzCoarseIndex:
                    value[0]=Qc98xxCoarseIndexGet(band_BG);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom2GHzFineIndex:
                    value[0]=Qc98xxFineIndexGet(band_BG);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom2GHzAdcDesiredSize:
                ...
                case Qc9KSetEeprom2GHzQuickDrop:
                    ...
                    break;
                case Qc9KSetEeprom2GHzMaxRetrainTimes:
                    value[0]=Qc98xxMaxRetrainTimesGet(band_BG);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom2GHzSettleTime:
                    value[0]=Qc98xxSettleTimeGet(band_BG);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom2GHzXatten1Db:
                ...
                case Qc9KSetEeprom2GHzXatten1Margin:
                    ...
                    break;
                case Qc9KSetEeprom2GHzObdbAux1Aux2:
                    status=Qc98xxObdbAux1Aux2Get(value, ix, &num, band_BG);
                    ReturnGet(ix,iy,iz,status, &done, &error, ip, index, value, num);
                    break;                    
                case Qc9KSetEeprom2GHzVoltageSlope:
				...
				case Qc9KSetEeprom5GHzMinCcaPwrChain:
					...
					break;
                case Qc9KSetEeprom5GHzCoarseIndex:
                    value[0]=Qc98xxCoarseIndexGet(band_A);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom5GHzFineIndex:
                    value[0]=Qc98xxFineIndexGet(band_A);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom5GHzAdcDesiredSize:
                ...
                case Qc9KSetEeprom5GHzQuickDrop:
                    ...
                    break;
                case Qc9KSetEeprom5GHzMaxRetrainTimes:
                    value[0]=Qc98xxMaxRetrainTimesGet(band_A);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom5GHzSettleTime:
                    value[0]=Qc98xxSettleTimeGet(band_A);
                    ReturnGet(ix,iy,iz,VALUE_OK, &done, &error, ip, index, value, 1);
                    break;
                case Qc9KSetEeprom5GHzXatten1DbLow:
                ...
                case Qc9KSetEeprom5GHzXatten1MarginHigh:
                    ...
                    break;
                case Qc9KSetEeprom5GHzObdbAux1Aux2Low:
                    status=Qc98xxObdbAux1Aux2LowGet(value, ix, &num, band_A);
                    ReturnGet(ix,iy,iz,status, &done, &error, ip, index, value, num);
                    break;
                case Qc9KSetEeprom5GHzObdbAux1Aux2Mid:
                    status=Qc98xxObdbAux1Aux2Get(value, ix, &num, band_A);
                    ReturnGet(ix,iy,iz,status, &done, &error, ip, index, value, num);
                    break;
                case Qc9KSetEeprom5GHzObdbAux1Aux2High:
                    status=Qc98xxObdbAux1Aux2HighGet(value, ix, &num, band_A);
                    ReturnGet(ix,iy,iz,status, &done, &error, ip, index, value, num);
                    break;                    
                case Qc9KSetEeprom5GHzVoltageSlope:
