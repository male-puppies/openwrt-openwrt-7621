...
extern int Qc98xxRxChainMany(void);
...
A_INT32 Qc98xxVoltSlopeSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	////printf("\nfix for 257878,257879\n");
	if(ix<WHAL_NUM_CHAINS)
	{
    		if (iBand==band_BG) {
...
A_INT32 Qc98xxPwrTuningCapsParamsSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
    	if(num == 1)
    	{
	    /* This is done only while the Tuning Cap Set function being called as part of XTAL Calibration as
	     * only one value is returned from the XTAL calibration or when user set only one tuning cap value
	     * as part of the set command.
	     */
	    EEPROM_BASE_POINTER->baseEepHeader.param_for_tuning_caps = (A_UINT8)value[0];
            EEPROM_CONFIG_DIFF_CHANGE(baseEepHeader.param_for_tuning_caps);
	    EEPROM_BASE_POINTER->baseEepHeader.param_for_tuning_caps_1 = (A_UINT8)value[0];
	    ...
	    EEPROM_BASE_POINTER->baseEepHeader.param_for_tuning_caps = (A_UINT8)value[0];
            EEPROM_CONFIG_DIFF_CHANGE(baseEepHeader.param_for_tuning_caps);
            EEPROM_BASE_POINTER->baseEepHeader.param_for_tuning_caps_1 = (A_UINT8)value[1];
            EEPROM_CONFIG_DIFF_CHANGE(baseEepHeader.param_for_tuning_caps_1);
...
/*
 *Function Name:Qc98xxEnableSmartAntennaSet
 *Parameters: value
 *Description: set EnableSmartAntenna flag in featureEnable 
 *             field of eeprom struct (bit 7)
 *Returns: zero
 */
A_INT32 Qc98xxEnableSmartAntennaSet(int value)
{
    if (value)
    {
        EEPROM_BASE_POINTER->baseEepHeader.opCapBrdFlags.featureEnable |= WHAL_FEATUREENABLE_SMART_ANTENNA_MASK;
    }
    else
    {
        EEPROM_BASE_POINTER->baseEepHeader.opCapBrdFlags.featureEnable &= (~WHAL_FEATUREENABLE_SMART_ANTENNA_MASK);
...
A_INT32 Qc98xxFlag1TxBurstSet(int value)
{
    if (value) 
    {
         EEPROM_BASE_POINTER->baseEepHeader.opCapBrdFlags.flag1 |= WHAL_BOARD_FLAG1_TX_BURST_MASK;
    }
    else
    {
         EEPROM_BASE_POINTER->baseEepHeader.opCapBrdFlags.flag1 &= (~WHAL_BOARD_FLAG1_TX_BURST_MASK);
...
A_INT32 Qc98xxPapdSet(int value)
{
    if (value) {
        //enable PAPRD 
        //Enable only for 5G now
        EEPROM_BASE_POINTER->baseEepHeader.opCapBrdFlags.boardFlags &= 
            (~( WHAL_BOARD_PAPRD_5G_DISABLE)); 
        EEPROM_BASE_POINTER->baseEepHeader.opCapBrdFlags.boardFlags |=
            ((WHAL_BOARD_PAPRD_2G_DISABLE));
...
A_INT32 Qc98xxXatten1DBSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
		...
		if (iBand==band_BG) {
	        ...
	        EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1DB[i].value2G, 1);
		} else {
			EEPROM_BASE_POINTER->freqModalHeader.xatten1DB[i].value5GMid = (A_UINT8)value[iv++];
	        EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1DB[i].value5GMid, 1);
		}
	}
	ResetForce();
    return 0;
...
A_INT32 Qc98xxXatten1MarginSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
		...
		if (iBand==band_BG) {
	        ...
	        EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1Margin[i].value2G, 1);
		} else {
			EEPROM_BASE_POINTER->freqModalHeader.xatten1Margin[i].value5GMid = (A_UINT8)value[iv++];
	        EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1Margin[i].value5GMid, 1);
		}
	}
	ResetForce();
    return 0;
...
A_INT32 Qc98xxXatten1DBLowSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
	    ...
	    EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1DB[i].value5GLow, 1);
	}
	ResetForce();
    return 0;
}
...
A_INT32 Qc98xxXatten1DBHighSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
	    ...
	    EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1DB[i].value5GHigh, 1);
	}
	ResetForce();
    return 0;
}
...
A_INT32 Qc98xxXatten1MarginLowSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
	    ...
	    EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1Margin[i].value5GLow, 1);
	}
	ResetForce();
    return 0;
}
...
A_INT32 Qc98xxXatten1MarginHighSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
	    ...
	    EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.xatten1Margin[i].value5GHigh, 1);
	}
	ResetForce();
    return 0;
}
...
A_INT32 Qc98xxCalFreqTGTcckSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	////printf("\nfix for 257867\n");
	if(ix < WHAL_NUM_11B_TARGET_POWER_CHANS)
...
A_INT32 Qc98xxCalFreqTGTHT80Set(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	//printf("\nfix for 257891\n");
	if(ix<WHAL_NUM_11A_80_TARGET_POWER_CHANS)
				  ...
				  int *baseChanged, A_UINT8 flag, int num, A_UINT16 pierth, A_UINT8 vhtIdx, A_BOOL is2GHz)
{
	...
	if (flag & FLAG_LAST)
	{
		...
		{
			//printf("\nFix 109266, 109267\n");
			if (targetPowerRateIndex < WHAL_NUM_VHT_TARGET_POWER_RATES)
			{
				if (shiftBit == 0 && ((targetPowerRateIndex+1)<WHAL_NUM_VHT_TARGET_POWER_RATES))
				{
...
A_INT32 Qc98xxCalTGTPwrHT20Set(double *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<iMaxPier; i++) 
    {
		...
		if (iBand==band_BG)
		{
			...
			{
				//printf("\nfix for 257868\n");
				if( ((j)>>1) < WHAL_NUM_11G_20_TARGET_POWER_RATES_DIV2 )
		...
		else
		{
			...
			{
				//printf("\nfix for 257869\n");
				if( ((j)>>1) < WHAL_NUM_11A_20_TARGET_POWER_RATES_DIV2 )
...
A_INT32 Qc98xxCalTGTPwrHT40Set(double *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<iMaxPier; i++) 
    {
		...
		if (iBand==band_BG)
		{
			...
			{
				//printf("\nfix for 257870\n");
				if( ((j)>>1) < WHAL_NUM_11G_40_TARGET_POWER_RATES_DIV2 )
		...
		else
		{
			...
			{
				//printf("\nfix for 257871\n");
				if( ((j)>>1) < WHAL_NUM_11A_40_TARGET_POWER_RATES_DIV2 )
...
A_INT32 Qc98xxCalTGTPwrHT80Set(double *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<iMaxPier; i++) 
    {
		...
		{
			//printf("\nfix for 257872\n");
			if( ((j) >> 1)< WHAL_NUM_11A_80_TARGET_POWER_RATES_DIV2)
...
int Qc98xxConfigAddrSet(unsigned int address, int low, int  high, unsigned int *value, int numVal, int prepost)
{
    ...
    for  (i = 0, pConfigAddr = eep->configAddr; (i < maxLen) && (pConfigAddr[i] != 0); i+=curEntryLen)
    {
        ...
        if (pConfigAddr[i] == addressMode)
        {
            ...
            {
		//printf("\nFix for 109280\n");
		if((i+1) < QC98XX_CONFIG_ENTRIES)
                	EEPROM_CONFIG_DIFF_CHANGE(configAddr[i+1]);
	...
	//printf("\nFix for 109281\n");
    if(i<QC98XX_CONFIG_ENTRIES)
    	EEPROM_CONFIG_DIFF_CHANGE(configAddr[i]);
    pConfigAddr += (i+1);
...
A_INT32 Qc98xxCoarseIndexSet(int value, int iBand)
{
	A_UINT8  value4;
	value4 = (A_UINT8)(value & 0xff);

    if (EEPROM_BASE_POINTER->baseEepHeader.eeprom_version != QC98XX_EEP_VER1)
    {
        if (iBand == band_both || iBand == band_BG)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_2G_INDX].coarseIndex = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_2G_INDX].coarseIndex);
        }
        if (iBand == band_both || iBand == band_A)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_5G_INDX].coarseIndex = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_5G_INDX].coarseIndex);
        }
    }
	ResetForce();
    return 0;
...
A_INT32 Qc98xxFineIndexSet(int value, int iBand)
{
	A_UINT8  value4;
	value4 = (A_UINT8)(value & 0xff);
    ...
    if (EEPROM_BASE_POINTER->baseEepHeader.eeprom_version != QC98XX_EEP_VER1)
    {
        if (iBand == band_both || iBand == band_BG)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_2G_INDX].fineIndex = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_2G_INDX].fineIndex);
        }
        if (iBand == band_both || iBand == band_A)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_5G_INDX].fineIndex = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_5G_INDX].fineIndex);
        }
    }
	ResetForce();
    return 0;
...
A_INT32 Qc98xxAdcDesiredSizeSet(int value, int iBand)
{
    ...
    if (EEPROM_BASE_POINTER->baseEepHeader.eeprom_version != QC98XX_EEP_VER1)
    {
        if (iBand == band_both || iBand == band_BG)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_2G_INDX].adcDesiredSize = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_2G_INDX].adcDesiredSize);
        }
        if (iBand == band_both || iBand == band_A)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_5G_INDX].adcDesiredSize = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_5G_INDX].adcDesiredSize);
        }
    }
	ResetForce();
...
A_INT32 Qc98xxQuickDropSet(int value, int iBand)
{
	A_INT8  value4;
	value4 = (A_INT8)value;
    ...
    if (EEPROM_BASE_POINTER->baseEepHeader.eeprom_version != QC98XX_EEP_VER1)
    {
        if (iBand == band_both || iBand == band_BG)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_2G_INDX].quickDrop = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_2G_INDX].quickDrop);
        }
        if (iBand == band_both || iBand == band_A)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_5G_INDX].quickDrop = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_5G_INDX].quickDrop);
        }
    }
	ResetForce();
    return 0;
...
A_INT32 Qc98xxMaxRetrainTimesSet(int value, int iBand)
{
	A_UINT8  value4;
	value4 = (A_UINT8)(value & 0xff);
    ...
    if (EEPROM_BASE_POINTER->baseEepHeader.eeprom_version != QC98XX_EEP_VER1)
    {
        if (iBand == band_both || iBand == band_BG)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_2G_INDX].maxRetrainTimes = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_2G_INDX].maxRetrainTimes);
        }
        if (iBand == band_both || iBand == band_A)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_5G_INDX].maxRetrainTimes = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_5G_INDX].maxRetrainTimes);
        }
    }
	ResetForce();
    return 0;
...
A_INT32 Qc98xxSettleTimeSet(int value, int iBand)
{
	A_UINT8  value4;
	value4 = (A_UINT8)(value & 0xff);
    ...
    if (EEPROM_BASE_POINTER->baseEepHeader.eeprom_version != QC98XX_EEP_VER1)
    {
        if (iBand == band_both || iBand == band_BG)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_2G_INDX].settleTime = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_2G_INDX].settleTime);
        }
        if (iBand == band_both || iBand == band_A)
        {
            EEPROM_BASE_POINTER->biModalHeader[BIMODAL_5G_INDX].settleTime = value4;
            EEPROM_CONFIG_DIFF_CHANGE(biModalHeader[BIMODAL_5G_INDX].settleTime);
        }
    }
	ResetForce();
    return 0;
...
A_INT32 Qc98xxObdbAux1Aux2Set(int *value, int ix, int iy, int iz, int num, int iBand)
{
	int i, iv=0;
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
		if (iv>=num)
			break;
		if (iBand==band_BG) {
			EEPROM_BASE_POINTER->freqModalHeader.obdbAux1Aux2[i].value2G = (A_UINT8)value[iv++];
	        EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.obdbAux1Aux2[i].value2G, 1);
		} else {
			EEPROM_BASE_POINTER->freqModalHeader.obdbAux1Aux2[i].value5GMid = (A_UINT8)value[iv++];
	        EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.obdbAux1Aux2[i].value5GMid, 1);
		}
	}
	ResetForce();
    return 0;
...
A_INT32 Qc98xxObdbAux1Aux2LowSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	int i, iv=0;
	if (iBand==band_BG)
		return 0;
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
		if (iv>=num)
			break;
		EEPROM_BASE_POINTER->freqModalHeader.obdbAux1Aux2[i].value5GLow = (A_UINT8)value[iv++];
	    EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.obdbAux1Aux2[i].value5GLow, 1);
	}
	ResetForce();
    return 0;
}

A_INT32 Qc98xxObdbAux1Aux2HighSet(int *value, int ix, int iy, int iz, int num, int iBand)
{
	...
	for (i=ix; i<WHAL_NUM_CHAINS; i++) {
		if (iv>=num)
			break;
		EEPROM_BASE_POINTER->freqModalHeader.obdbAux1Aux2[i].value5GHigh = (A_UINT8)value[iv++];
	    EEPROM_CONFIG_DIFF_CHANGE_ARRAY(freqModalHeader.obdbAux1Aux2[i].value5GHigh, 1);
	}
	ResetForce();
    return 0;
}
