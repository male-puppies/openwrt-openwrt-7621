...
void print9300_5GHzHeader(int client, const OSPREY_MODAL_EEP_HEADER   *pModalHeader, const OSPREY_BASE_EXTENSION_2 *base_ext2, const OSPREY_BASE_EXTENSION_1 *base_ext1)
{
	...
	SformatOutput(buffer, MBUFFER-1," | Spur Channels:           %04d, %04d, %04d, %04d, %04d                 |",
        ...
        SformatOutput(buffer, MBUFFER-1," | Obdboffst_aux1_0      %3d         |  Obdboffst_aux1_1       %3d       |",
                 pModalHeader->obdboffst_aux1[0],
				 pModalHeader->obdboffst_aux1[1]
				 );
		ErrorPrint(NartOther, buffer);

		SformatOutput(buffer, MBUFFER-1," | Obdboffst_aux1_2      %3d         |                                   |",
                 pModalHeader->obdboffst_aux1[2]
				 );
		ErrorPrint(NartOther, buffer);

        SformatOutput(buffer, MBUFFER-1," | Obdboffst_aux2_0      %3d         |  Obdboffst_aux2_1       %3d       |",
                 pModalHeader->obdboffst_aux2[0],
				 pModalHeader->obdboffst_aux2[1]
				 );
		ErrorPrint(NartOther, buffer);

		SformatOutput(buffer, MBUFFER-1," | Obdboffst_aux2_2      %3d         |                                   |",
                 pModalHeader->obdboffst_aux2[2]
				 );
		ErrorPrint(NartOther, buffer);
...
void print9300_2GHzHeader(int client, const OSPREY_MODAL_EEP_HEADER   *pModalHeader, const OSPREY_BASE_EXTENSION_2 *base_ext2)
{
	...
	if(AR_SREV_HONEYBEE(AH)) {
			SformatOutput(buffer, MBUFFER-1," | Temp Slope           %3d,%3d      |  Volt Slope             %3d       |",
				base_ext2->temp_slope_low,
				pModalHeader->temp_slope,
				pModalHeader->voltSlope
				);
			...
			/*Scorpion & Dragonfly has individual tempslope Registers*/
            ...
            for (j = 0; j < 4; j++) {
					...
					for(i=0;i<8;i++){
						...
						if (strlen(buffer) < (sizeof(buffer)-1))
                					strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));	//Fix for 112559
                }
				if (strlen(buffer) < (sizeof(buffer)-1))
					strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));		//Fix for 112560
				ErrorPrint(NartOther, buffer);
	...
	ErrorPrint(NartOther,buffer);
            ...
            for (j = 0; j < 4; j++) {
					...
					for(i=0;i<3;i++){
						SformatOutput(buffer2, MBUFFER-1,"|        %2d.%d        ", pVals[i].t_pow2x[j]/2, (pVals[i].t_pow2x[j] % 2) * 5);
						if (strlen(buffer) < (sizeof(buffer)-1))
							strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//Fix for 115261
                }
				if (strlen(buffer) < (sizeof(buffer)-1))
					strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));	//fix for 115262
				ErrorPrint(NartOther, buffer);
	...
	ErrorPrint(NartOther,buffer);
            ...
            for (j = 0; j < 4; j++) {
					...
					for(i=0;i<2;i++){
						SformatOutput(buffer2, MBUFFER-1,"|        %2d.%d        ", pVals[i].t_pow2x[j]/2, (pVals[i].t_pow2x[j] % 2) * 5);
						if (strlen(buffer) < (sizeof(buffer)-1))
							strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//fix for 112563
                }
				if (strlen(buffer) < (sizeof(buffer)-1))
					strncat(buffer,"|====================|",(sizeof(buffer)-1-strlen(buffer)));		//Fix for 112564
				ErrorPrint(NartOther, buffer);
	...
	ErrorPrint(NartOther,buffer);
            ...
            for (j = 0; j < 24; j++) {
					...
					for(i=0;i<8;i++){
						...
						if (strlen(buffer) < (sizeof(buffer)-1))
							strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//Fix for 112565
                }
				if (strlen(buffer) < (sizeof(buffer)-1))
					strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));		//Fix for 112566
				ErrorPrint(NartOther, buffer);
	...
	ErrorPrint(NartOther,buffer);
            ...
            for (j = 0; j < 24; j++) {
					...
					for(i=0;i<8;i++){
						SformatOutput(buffer2, MBUFFER-1,"| %2d.%d  ", pVals[i].t_pow2x[mapRate2Index[j]]/2, (pVals[i].t_pow2x[mapRate2Index[j]] % 2) * 5);
						if (strlen(buffer) < (sizeof(buffer)-1))
							strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//Fix for 112567
                }
				if (strlen(buffer) < (sizeof(buffer)-1))
					strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));		//Fix for 112568
				ErrorPrint(NartOther, buffer);
	...
	ErrorPrint(NartOther,buffer);
            ...
            for (j = 0; j < 24; j++) {
					...
					for(i=0;i<3;i++){
						SformatOutput(buffer2, MBUFFER-1,"|        %2d.%d        ", pVals[i].t_pow2x[mapRate2Index[j]]/2, (pVals[i].t_pow2x[mapRate2Index[j]] % 2) * 5);
						if (strlen(buffer) < (sizeof(buffer)-1))
							strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//Fix for 112569
                }

				if (strlen(buffer) < (sizeof(buffer)-1))
					strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));			//Fix for 112570
				ErrorPrint(NartOther, buffer);
	...
	ErrorPrint(NartOther,buffer);
            ...
            for (j = 0; j < 24; j++) {
					...
					for(i=0;i<3;i++){
						SformatOutput(buffer2, MBUFFER-1,"|        %2d.%d        ", pVals[i].t_pow2x[mapRate2Index[j]]/2, (pVals[i].t_pow2x[mapRate2Index[j]] % 2) * 5);
						if (strlen(buffer) < (sizeof(buffer)-1))
                					strncat(buffer,buffer2,  (sizeof(buffer)-1-strlen(buffer)));		//Fix for 112571
                }
				if (strlen(buffer) < (sizeof(buffer)-1))
					strncat(buffer,"|",(sizeof(buffer)-1-strlen(buffer)));		//Fix for 112572
				ErrorPrint(NartOther, buffer);
