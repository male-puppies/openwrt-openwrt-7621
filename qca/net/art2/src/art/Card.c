...
int CardResetIfNeeded(int frequency, int txchain, int rxchain, int force, int bandwidth)
{
    ...
    rxchainuse = DeviceRxChainMask();
   	...
   	{	
   		...
   		// Changing the above way to take care of the rxchainmask just before the reset in the device specific file , so that we can make it generic
		// for eg visit Ar9300Reset function in Ar9300Device.c . This is changed to fix EV#130174
        error=CardResetDo(frequency, txchain, rxchain, bandwidthUse);
   		if(error!=0)
   		{
 		...
 		if(error!=0 || NoiseFloorBad(rxchainuse, bandwidthUse))		// ####need to get correct chain mask
   		{
        	...
        	if(ChannelFind(frequency+1,10000,bandwidth,&tfrequency,&bandwidthTemp))
   			...
   			error=CardResetDo(tfrequency, txchain, rxchain, bandwidthTemp);
   			//
			...
			if(index>=0)
			{
				...
				switch(code) 
				{
					...
					case LinkParameterRxChain:
						ngot=ParseHexList(ip,name,&rxchain,&ResetParameter[index]);
						if(ngot<=0)
						{
							error++;
						}
...
// Following block will be removed after LInux dynamic library is implemented
	...
	*/
    if (((devid == AR6004_DEVID)||(devid == AR6006_DEVID)) && (calmem != 0))
	{
		...
		//printf("\nFix for 108291\n");
		snprintf(buffer, sizeof(buffer),"%d\0", calmem);
	...
	{	
		...
		char *ret=NULL;
		//printf("fix 256529:\n");
		if((ret=DeviceIniVersion(devid))!=0)
		{
    			SformatOutput(buffer,MBUFFER-1,"|set|IniVersion|%s|",ret);
...
void DeviceDataSend(void)
{
    char buffer[MBUFFER];
    char *ret=NULL;
    //printf("fix :256527,256528,256530256527,256528,256530\n");
    if((ret = DeviceName())!= 0)
    {
   	 SformatOutput(buffer,MBUFFER-1,"|set|DeviceName|%s|",ret);
   	 ErrorPrint(NartData,buffer);
    }
    if((ret = DeviceVersion())!= 0)
    {
	SformatOutput(buffer,MBUFFER-1,"|set|DeviceVersion|%s|",ret);
    	ErrorPrint(NartData,buffer);
    }
    if((ret = DeviceBuildDate())!= 0)
    {
    	SformatOutput(buffer,MBUFFER-1,"|set|DeviceBuildDate|%s|",ret);
   	 ErrorPrint(NartData,buffer);
    }
#ifdef DYNAMIC_DEVICE_DLL
    if((ret = DeviceFullName())!= 0)
