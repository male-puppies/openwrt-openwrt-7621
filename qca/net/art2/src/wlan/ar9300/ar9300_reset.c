...
ar9300_spur_mitigate_mrc_cck(struct ath_hal *ah, HAL_CHANNEL *chan)
{
    ...
    int synth_freq;
    int range = 10;
    int max_spurcounts = OSPREY_EEPROM_MODAL_SPURS-1; 
...
static void ar9300_manual_pk_cal(struct ath_hal *ah)
{
	int bit=5; 
...
#define OFF_LOWER_LT 8
	...
	if (AR_SREV_DRAGONFLY(ah) || AR_SREV_SCORPION(ah) || AR_SREV_HONEYBEE(ah)) {
		ar9300_manual_pk_cal(ah);
    }
...
reset_begin:    
    ...
    *status = HAL_OK;
    ...
    if (curchan && (ahp->ah_chip_full_sleep != true)) {
        ...
        if(ahp->ah_chip_reset_done){
        	...
        	/*
         	 ...
         	 */
        	if (chan && (ap->ah_curchan->channel != chan->channel))
    ...
    if ((AR_SREV_SCORPION(ah) || AR_SREV_HONEYBEE(ah)) && chan && curchan && (chan->channel == curchan->channel) &&
     ...
     */
    if (chan && ichan && !is_scan && (!ap->ah_curchan ||
    ...
    if (is_scan) {
        ...
        if (chan && ahp->ah_rx_cal_complete &&
...
#if ATH_SUPPORT_CAL_REUSE
        if (ichan && (AH_PRIVATE(ah)->ah_config.ath_hal_cal_reuse & ATH_CAL_REUSE_ENABLE) &&
...
#endif
    {
        if (chan && ichan && ar9300_channel_change(ah, chan, ichan, macmode)) {
            chan->channel_flags = ichan->channel_flags;
            chan->priv_flags = ichan->priv_flags;
            AH_PRIVATE(ah)->ah_curchan->ah_channel_time = 0;
            AH_PRIVATE(ah)->ah_curchan->ah_tsf_last = ar9300_get_tsf64(ah);
    ...
    if (chan && !ar9300_chip_reset(ah, chan)) {
    ...
    /*
    * IR-090559:
    * Disable tx_iq_cal to fix noise floor failure issue in Scorpion for QuarterRate.
    */  
    if(chan && IS_CHAN_QUARTER_RATE(chan) && AR_SREV_SCORPION(ah))
    {       
	...
	*/
    if ((tx_iq_cal_disable) && (AR_SREV_DRAGONFLY(ah) || (chan && AR_SREV_SCORPION(ah) && IS_CHAN_QUARTER_RATE(chan)))) {
        OS_REG_CLR_BIT(ah, AR_PHY_TX_IQCAL_CONTROL_0(ah),
                AR_PHY_TX_IQCAL_CONTROL_0_ENABLE_TXIQ_CAL);
    ...
    if (chan && (IS_CHAN_OFDM(chan) || IS_CHAN_HT(chan))) {
        ar9300_set_delta_slope(ah, ichan);
    }
    ...
    if (ichan && !ar9300_eeprom_set_board_values(ah, ichan)) {
        HDPRINTF(ah, HAL_DBG_EEPROM,
     ...
     */
    if (ichan && !ahp->ah_rf_hal.set_channel(ah, ichan)) {
        FAIL(HAL_EIO);
    }
...
#undef REG_WRITE
    ...
    #ifndef ATH_NF_PER_CHAN
	    if (chan && ichan && First_NFCal(ah, ichan, is_scan, chan)){
