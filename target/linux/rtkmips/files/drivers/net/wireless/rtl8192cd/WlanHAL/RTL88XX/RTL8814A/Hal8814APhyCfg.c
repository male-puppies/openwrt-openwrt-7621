...
)
{
	...
	PHY_SetBBReg(priv, 0x834, 0x0000000F, 0x6);      // 0x834 reset default setting
	PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0xFFFFFFFF, 0x79A0EA08);      // 0x830 reset default setting
	if(priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_80){
		...
		case ANTENNA_D:
	        ...
	        PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0x000000FF, 0x0a);      // 0x830
		break;
		...
		case ANTENNA_CD:
	        PHY_SetBBReg(priv, REG_BB_CCAONSEC_AC, 0x00000001, 0x1);      // Enable 2ndCCA
	        PHY_SetBBReg(priv, REG_BB_AGC_TABLE_AC, 0xFFFF0000, 0x7578);    // 0x82C[23:20] = 7, PWDB_TH_QB, 0x82C[19:16] = 8, PWDB_TH_HB
		...
		case ANTENNA_D:
			...
			PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0x7C0000, 0xe);           // 830[22:18] = 5b'01110
	        PHY_SetBBReg(priv, 0x844, 0x0F000000, 0xb);                     //0x844[27:24]=0xb
	        PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0x000000FF, 0x0a);      // 0x830
			break;
		case ANTENNA_AB: // xT2R
		...
		case ANTENNA_CD:
			PHY_SetBBReg(priv, REG_BB_AGC_TABLE_AC, 0xFFFF0000, 0x7598);         // 82C[23:16]  reset default
			PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0x7C0000, 0x8);           // 830[22:18] = 5b'01000
	        PHY_SetBBReg(priv, 0x844, 0x0F000000, 0xd);                     //0x844[27:24]=0xd
	        PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0x000000FF, 0x0a);      // 0x830
	        break;
		...
		default:
	        ...
	        if(Current_band == RF88XX_BAND_ON_2_4G) {
				...
				PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0x000000FF, 0x0b);      // 0x830
	        }else{
		...
		default:
			...
			if (ulAntennaRx==ANTENNA_ABCD){
				if(Current_band == RF88XX_BAND_ON_2_4G) {
					...
					PHY_SetBBReg(priv, 0x834, 0x0000000F, 0xa);      // 0x834
					PHY_SetBBReg(priv, REG_BB_PWED_TH_AC, 0xFFFFFFFF, 0x79a14a0a);      // 0x830
		        }else{
	...
	if(Temp_Flag){
		if(HAL_VAR_CURRENTCHANNELBW==HT_CHANNEL_WIDTH_20){
			...
			if (((Adapter->pshare->rf_ft_var.mp_specific==1)&&((Adapter->pshare->mp_antenna_rx==ANTENNA_A)||(Adapter->pshare->mp_antenna_rx==ANTENNA_B)||(Adapter->pshare->mp_antenna_rx==ANTENNA_C)||(Adapter->pshare->mp_antenna_rx==ANTENNA_D)))
				...
				PHY_SetBBReg(Adapter, REG_BB_AGC_TABLE_AC, 0x00FF0000, 0x78);
