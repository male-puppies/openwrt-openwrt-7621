...
#define MU_SOUNDING_TIMEOUT			100	// unit: ms
...
#if defined(CONFIG_WLAN_HAL_8822BE)
#define BEAMFORMEE_ENTRY_NUM		((priv->pshare->rf_ft_var.num_su_txbfee_entry) + (priv->pshare->rf_ft_var.num_mu_txbfee_entry))
#define BEAMFORMER_ENTRY_NUM		2 //(MAX_NUM_BEAMFORMER_SU + MAX_NUM_BEAMFORMER_MU)
...
typedef struct _RT_BEAMFORMING_INFO {
	...
	u1Byte					SetHalBFEnterOnDemandCnt;
	u1Byte					SetHalBFLeaveOnDemandCnt;
...
//#endif
	...
	unsigned char		num_su_txbfee_entry;
	unsigned char		num_mu_txbfee_entry;
