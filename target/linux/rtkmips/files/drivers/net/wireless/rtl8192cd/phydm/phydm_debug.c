...
)
{
	...
	case PHYDM_LA_MODE:
		...
		if (pDM_Odm->SupportICType & PHYDM_IC_SUPPORT_LA_MODE) {
			...
			if ((strcmp(input[1], help) == 0) && (input_num == 2)) {
				PHYDM_SNPRINTF((output+used, out_len-used, "{En} {0:BB,1:MAC} {BB:DbgPort[bit],MAC:ok/fail/cca} {RptFormat} {TrigTime} {PollingTime} {DbgPort} {0:P_Edge, 1:N_Edge}\n"));
				/**/
			} else if ((bEnableLaMode == 1) && (input_num == 9)) {
				...
				PHYDM_SNPRINTF((output+used, out_len-used, "TriggerTime = %d, PollingTime = %d, sampling rate = %d\n", TriggerTime, PollingTime, sampling_rate));
			} else {
				#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	...
	#else
		PHYDM_SNPRINTF((output+used, out_len-used, "This Platform doesn't support LA mode\n"));
	#endif
		break;
